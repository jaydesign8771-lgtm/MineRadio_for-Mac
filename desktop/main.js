...
const LOCAL_AUDIO_EXT_RE = /\.(mp3|flac|wav|ogg|m4a)$/i;
const LOCAL_IMAGE_EXT_RE = /\.(jpg|jpeg|png|webp)$/i;
const LOCAL_LYRIC_EXT_RE = /\.(lrc|txt|text|lyric|lyrics)$/i;
...
// d3d11 仅 Windows 可用；macOS 用 metal，避免 GPU 进程初始化失败
app.commandLine.appendSwitch('use-angle', process.platform === 'win32' ? 'd3d11' : 'metal');
...
function walkLocalMusicFiles(dirPath, out = []) {
  let entries = [];
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      walkLocalMusicFiles(fullPath, out);
      continue;
    }
...
function pickBestLocalCover(files) {
  if (!files.length) return '';
  const preferred = files.find((filePath) => /^(cover|folder|album|front|cover\.art|albumart)$/i.test(path.basename(filePath, path.extname(filePath))));
  return preferred || files[0];
}

function groupLocalMusicFolders(folderPaths = []) {
  const roots = Array.from(new Set((Array.isArray(folderPaths) ? folderPaths : []).map((item) => path.resolve(String(item || '')))))
    .filter((folderPath) => {
      try {
  ...
  for (const rootPath of roots) {
    ...
    const files = walkLocalMusicFiles(rootPath, []);
    for (const filePath of files) {
      const relPath = path.relative(rootPath, filePath);
      if (!relPath || relPath.startsWith('..')) continue;
      const segments = relPath.split(path.sep).filter(Boolean);
      const groupName = segments.length > 1 ? segments[0] : '__ROOT__';
      const groupId = `${rootPath}::${groupName}`;
      const groupTitle = groupName === '__ROOT__' ? rootName : groupName;
      const groupSubtitle = groupName === '__ROOT__' ? '根目录歌曲' : rootName;
      let group = groups.get(groupId);
      if (!group) {
        group = { id: groupId, title: groupTitle, subtitle: groupSubtitle, rootPath, groupName, audioFiles: [], imageFiles: [], lyricMap: {} };
        groups.set(groupId, group);
      }
      const baseLower = path.basename(filePath, path.extname(filePath)).toLowerCase();
      if (LOCAL_AUDIO_EXT_RE.test(filePath)) {
        let stat = null;
      ...
      } else if (LOCAL_IMAGE_EXT_RE.test(filePath)) {
        group.imageFiles.push(filePath);
      } else if (LOCAL_LYRIC_EXT_RE.test(filePath)) {
        try {
          const text = fs.readFileSync(filePath, 'utf8');
          if (text && text.trim()) group.lyricMap[baseLower] = text;
        } catch (e) {}
      }
    }
  }
  return {
    ...
    groups: Array.from(groups.values()).map((group) => {
      const coverPath = pickBestLocalCover(group.imageFiles);
      const cover = coverPath ? pathToFileURL(coverPath).toString() : '';
      const songs = group.audioFiles
        ...
        .map((item) => ({
          ...
          artist: group.title || '本地音乐',
          cover,
          duration: 0,
          localPath: item.filePath,
          localUrl: pathToFileURL(item.filePath).toString(),
          localKey: `path:${item.filePath}:${item.stat ? item.stat.size : 0}:${item.stat ? Number(item.stat.mtimeMs || 0) : 0}`,
          localRootPath: group.rootPath,
          localGroup: group.title,
          relativePath: item.relPath,
          lyricText: group.lyricMap[item.baseLower] || '',
        }));
      return {
        id: group.id,
        title: group.title,
    ...
    }).filter((group) => group.songCount > 0)
...
function createTray() {
  if (tray) return;
  let img = null;
  try { img = nativeImage.createFromPath(APP_ICON_PNG); } catch (e) { img = null; }
  if (img && !img.isEmpty()) {
    try { img = img.resize({ width: 22, height: 22 }); } catch (e) {}
  ...
  const menu = Menu.buildFromTemplate([
    ...
    { type: 'separator' },
    { label: '退出 Mineradio', click: () => { isQuitting = true; app.quit(); } }
  ]);
  tray.setToolTip('Mineradio');
  tray.setContextMenu(menu);
  tray.on('click', () => {
    if (process.platform === 'darwin') tray.popUpContextMenu(menu);
...
ipcMain.handle('mineradio-pick-local-music-folders', async (event) => {
  try {
    const owner = getSenderWindow(event);
    const result = await dialog.showOpenDialog(owner, {
      title: '选择本地音乐文件夹',
    ...
    return { ok: true, folderPaths: result.filePaths };
  } catch (e) {
    return { ok: false, error: e.message || 'LOCAL_FOLDER_PICK_FAILED' };
  }
});

ipcMain.handle('mineradio-scan-local-music-folders', async (_event, folderPaths = []) => {
  try {
    // ponytail: synchronous scan keeps the diff small; move off-thread only if huge libraries make it feel slow.
    return Object.assign({ ok: true }, groupLocalMusicFolders(folderPaths));
  } catch (e) {
    return { ok: false, error: e.message || 'LOCAL_FOLDER_SCAN_FAILED', roots: [], groups: [] };
  }
});
