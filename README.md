...
Mineradio 是一款桌面沉浸式音乐播放器。本仓库是 Mac 版本工程，把本地音乐导入、搜索播放、歌词舞台、粒子视觉和 3D 歌单架组合成一个更接近现场感的私人音乐空间。

## Mac 版本

当前 Mac 版支持本地文件夹导入、歌词文件识别、双语歌词快捷控制和 macOS 风格窗口控制。正式安装包会放在 GitHub Releases。

## 下载或安装被拦截怎么办

小众 Electron 桌面软件、未签名安装包有时会被浏览器或 macOS Gatekeeper 提示风险。请先确认安装包来自本仓库 GitHub Release。

1. 浏览器下载栏提示风险时，打开下载列表，选择保留。
2. macOS 提示无法打开时，请在系统设置的隐私与安全性里允许打开。
3. 如果安全软件明确显示高危或已经隔离，不要强行运行；删除该文件后重新从 GitHub Release 下载，仍然异常请带截图反馈给作者。
...
Mac 用户可以在 GitHub Releases 中下载 DMG 安装包。

正式分发以 `Mineradio-1.1.1.dmg` 为准，不建议直接下载打包目录作为正式分发包。
